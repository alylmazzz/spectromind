...
// ---------------------------------------------------------------------------
// Context: everything the evaluators can access
// ---------------------------------------------------------------------------
...
export interface RuleEvalResult {
  ...
  autofix?: { patch_type?: string; patch_hint?: string };
  rule_version?: string;
}
...
function ruleApplies(rule: RuleEntry, engineId: string): boolean {
  const engines = rule.applies_to_engines;
  if (!engines || engines.length === 0) return true;
  return engines.includes(engineId);
}

function peaksInRange(peaks: ObservedPeak[] | undefined, min: number, max: number): ObservedPeak[] {
  if (!peaks) return [];
  return peaks.filter(p => p.ppm >= min && p.ppm <= max);
}

function bandsInRange(bands: ObservedIRBand[] | undefined, min: number, max: number): ObservedIRBand[] {
  if (!bands) return [];
  return bands.filter(b => b.cm >= min && b.cm <= max);
}

function msInRange(peaks: ObservedMSPeak[] | undefined, mz: number, tol: number): ObservedMSPeak[] {
  if (!peaks) return [];
  return peaks.filter(p => Math.abs(p.mz - mz) <= tol);
}

function peaks2dInRegion(
  peaks: Observed2DPeak[] | undefined,
  f1Min: number, f1Max: number,
  f2Min: number, f2Max: number
): Observed2DPeak[] {
  if (!peaks) return [];
  return peaks.filter(
    p => p.f1 >= f1Min && p.f1 <= f1Max && p.f2 >= f2Min && p.f2 <= f2Max
  );
}

function skipResult(rule: RuleEntry, reason: string): RuleEvalResult {
  const status = (rule.status_on_skip as RuleEvalStatus) ?? 'SKIP';
  return {
...
function pickMeta<T>(ctx: EvalContext, key: keyof EvalMetadataContract, fallback?: T): T | undefined {
  if (ctx.meta && key in ctx.meta) return ctx.meta[key] as T;
  return fallback;
}
...
type Evaluator = (rule: RuleEntry, ctx: EvalContext) => RuleEvalResult;

const EVALUATORS: Record<string, Evaluator> = {};

function register(ruleId: string, fn: Evaluator) {
...
register('FORMULA_DBE_NEGATIVE_FATAL', (rule, ctx) => {
  ...
  const status: RuleEvalStatus = dbe < 0 ? 'FATAL' : 'PASS';
...
register('FORMULA_DBE_PARITY', (rule, ctx) => {
  ...
  const status: RuleEvalStatus = isHalfInt ? 'PASS' : 'WARN';
...
register('CROSS_AROMATICITY_CONSENSUS_RULE', (rule, ctx) => {
  if (!ctx.graphFeatures) return skipResult(rule, 'no graph features');
  const isAromatic = ctx.graphFeatures.nC_aromatic > 0;
  if (!isAromatic) return makeResult(rule, 'PASS', { aromatic_ring: false, note: 'no aromatic ring in structure' });
  ...
  let confirmed = 0;
  let total = 0;
  if (h1Peaks) { total++; if (h1Aromatic && h1Aromatic.length > 0) confirmed++; }
  if (ctx.c13Peaks) { total++; if (c13ArPeaks && c13ArPeaks.length > 0) confirmed++; }
  if (ctx.ftirPeaks) { total++; if (irArBands && irArBands.length > 0) confirmed++; }

  if (total === 0) return skipResult(rule, 'no observed spectra for consensus');

  const status: RuleEvalStatus = confirmed >= 2 ? 'PASS' : confirmed >= 1 ? 'WARN' : 'FAIL';
  return makeResult(rule, status, {
    aromatic_ring: true,
...
register('CROSS_CARBONYL_CONSENSUS', (rule, ctx) => {
  if (!ctx.graphFeatures.has_carbonyl) return makeResult(rule, 'PASS', { carbonyl_in_structure: false });
  ...
  let confirmed = 0;
  let total = 0;
  if (ctx.c13Peaks) { total++; if (c13Car && c13Car.length > 0) confirmed++; }
  if (ctx.ftirPeaks) { total++; if (irCar && irCar.length > 0) confirmed++; }

  if (total === 0) return skipResult(rule, 'no 13C or IR data');
  const status: RuleEvalStatus = confirmed >= 1 ? 'PASS' : 'WARN';
  return makeResult(rule, status, {
...
register('CROSS_OH_CONSENSUS', (rule, ctx) => {
  ...
  if (exch === 0) return makeResult(rule, 'PASS', { exchangeable_OH: 0 });
  const irOH = ctx.ftirPeaks ? bandsInRange(ctx.ftirPeaks, 3200, 3550) : null;
  if (!ctx.ftirPeaks) return skipResult(rule, 'no IR data');
  const status: RuleEvalStatus = irOH && irOH.length > 0 ? 'PASS' : 'WARN';
...
register('CROSS_DBE_IMPLIES_SP2_SIGNALS', (rule, ctx) => {
  const dbe = ctx.dbeValue ?? ctx.graphFeatures.dbe;
  const thresh = (rule.thresholds as Record<string, number>)?.dbe_high_threshold ?? 6;
  if (dbe < thresh) return makeResult(rule, 'PASS', { dbe, threshold: thresh, note: 'DBE below threshold' });
  ...
  if (!ctx.h1?.peaks && !ctx.c13Peaks) return skipResult(rule, 'no 1H or 13C data');
...
register('DBE_AROMATIC_MIN_1H_INTEGRAL', (rule, ctx) => {
  const dbe = ctx.dbeValue ?? ctx.graphFeatures.dbe;
  const thresh = (rule.thresholds as Record<string, number>)?.dbe_aromatic_threshold ?? 4;
  if (dbe < thresh) return makeResult(rule, 'PASS', { dbe, note: 'DBE below aromatic threshold' });
  if (!ctx.h1?.peaks) return skipResult(rule, 'no 1H data');
  const arPeaks = peaksInRange(ctx.h1.peaks, 6.0, 9.0);
  const arIntegral = arPeaks.reduce((sum, p) => sum + (p.integral ?? 1), 0);
  const minRequired = (rule.thresholds as Record<string, number>)?.min_aromatic_H_if_aromatic_candidate ?? 2;
  const status: RuleEvalStatus = arIntegral >= minRequired ? 'PASS' : 'WARN';
...
register('H1_REQUIRE_AROMATIC_REGION_IF_AROMATIC_CH', (rule, ctx) => {
  if (ctx.graphFeatures.nH_aromatic === 0) return makeResult(rule, 'PASS', { aromatic_CH_count: 0 });
  if (!ctx.h1?.peaks) return skipResult(rule, 'no 1H data');
...
register('H1_INTEGRAL_TOTAL_MATCH_NONEXCH', (rule, ctx) => {
  ...
  if (sumIntegrals === 0) return skipResult(rule, 'no integral data');
  const ratio = sumIntegrals / Math.max(1, expected);
  const status: RuleEvalStatus = ratio >= 0.7 && ratio <= 1.3 ? 'PASS' : 'WARN';
  return makeResult(rule, status, {
...
register('H1_INTEGRAL_PARSING_SINGLE_AUTHORITY', (rule, ctx) => {
  if (!ctx.h1?.peaks) return skipResult(rule, 'no 1H data');
  if (ctx.h1.peaks.length === 0) return skipResult(rule, '1H peak list empty');
  ...
  const issues: string[] = [];
  if (!hasIntegrals) issues.push('No peaks have integral values assigned');
  if (!allPositive) issues.push('Negative integral values detected');
  const status: RuleEvalStatus = hasIntegrals && allPositive ? 'PASS' : 'WARN';
  return makeResult(rule, status, {
...
register('H1_REQUIRE_VINYLIC_REGION_IF_ALKENE_CH', (rule, ctx) => {
  ...
  if (sp2NonAr === 0) return makeResult(rule, 'PASS', { alkene_ch: 0 });
  if (!ctx.h1?.peaks) return skipResult(rule, 'no 1H data');
...
register('H1_ETHYL_MOTIF', (rule, ctx) => {
  if (!ctx.h1?.peaks || ctx.h1.peaks.length < 2) return skipResult(rule, 'insufficient 1H data');
...
register('H1_TERT_BUTYL_MOTIF', (rule, ctx) => {
  if (!ctx.h1?.peaks) return skipResult(rule, 'no 1H data');
  const tbCandidates = ctx.h1.peaks.filter(
    p => (p.mult === 's' || p.mult === 'singlet') && (p.integral ?? 0) >= 8 && p.ppm >= 0.8 && p.ppm <= 1.5
  );
  return makeResult(rule, 'INFO', {
...
register('H1_REQUIRE_BENZYLIC_REGION_IF_PRESENT', (rule, ctx) => {
  if (ctx.graphFeatures.nC_aromatic === 0) return makeResult(rule, 'PASS', { benzylic_expected: false });
  if (!ctx.h1?.peaks) return skipResult(rule, 'no 1H data');
...
register('H1_SOLVENT_IMPURITY_DETECTION', (rule, ctx) => {
  if (!ctx.h1?.peaks) return skipResult(rule, 'no 1H data');
  const SOLVENT_PEAKS: Record<string, number[]> = {
  ...
  const nearSolvent = expected.flatMap(sp =>
    ctx.h1!.peaks!.filter(p => Math.abs(p.ppm - sp) < 0.05)
  );
  return makeResult(rule, 'INFO', {
    solvent,
...
register('C13_CLASS_EXPECTED_VS_OBSERVED', (rule, ctx) => {
  if (ctx.c13Signals === undefined && !ctx.c13Peaks) return skipResult(rule, 'no 13C data');
  ...
  const status: RuleEvalStatus = ratio >= 0.5 && ratio <= 1.2 ? 'PASS' : 'WARN';
  return makeResult(rule, status, {
...
register('C13_AROMATIC_OVERLAP_RELAXATION', (rule, ctx) => {
  if (ctx.graphFeatures.nC_aromatic === 0) return makeResult(rule, 'PASS', { aromatic_carbons: 0 });
  return makeResult(rule, 'INFO', {
...
register('C13_REQUIRE_SP2_REGION_IF_SP2_C_PRESENT', (rule, ctx) => {
  ...
  if (sp2Total === 0) return makeResult(rule, 'PASS', { sp2_C_total: 0 });
  if (!ctx.c13Peaks) return skipResult(rule, 'no 13C peak data');
...
register('C13_REQUIRE_CARBONYL_REGION_IF_CARBONYL_PRESENT', (rule, ctx) => {
  if (!ctx.graphFeatures.has_carbonyl) return makeResult(rule, 'PASS', { carbonyl_C_count: 0 });
  if (!ctx.c13Peaks) return skipResult(rule, 'no 13C peak data');
...
register('C13_SYMMETRY_EXPECTATION_UNIQUE_SIGNAL_COUNT', (rule, ctx) => {
  if (!ctx.c13Peaks && ctx.c13Signals === undefined) return skipResult(rule, 'no 13C data');
  const observed = ctx.c13Signals ?? ctx.c13Peaks!.length;
  const expected = ctx.graphFeatures.expected_unique_carbons_approx;
  const ratio = observed / Math.max(1, expected);
  const minRatio = (rule.thresholds as Record<string, number>)?.min_ratio_unique ?? 0.45;
  const status: RuleEvalStatus = ratio >= minRatio ? 'PASS' : 'WARN';
  return makeResult(rule, status, {
...
register('C13_NITRILE_RANGE_IF_CYANO_PRESENT', (rule, ctx) => {
  if (!ctx.graphFeatures.has_nitrile) return makeResult(rule, 'PASS', { has_nitrile: false });
  if (!ctx.c13Peaks) return skipResult(rule, 'no 13C peak data');
...
register('HSQC_CH_COUNT_MATCH_MIN_RATIO', (rule, ctx) => {
  if (!ctx.hsqcPeaks) return skipResult(rule, 'no HSQC data');
  const protonatedC = ctx.graphFeatures.nC_protonated;
  const ratio = ctx.hsqcPeaks.length / Math.max(1, protonatedC);
  const minRatio = (rule.thresholds as Record<string, number>)?.min_ratio ?? 0.4;
  const status: RuleEvalStatus = ratio >= minRatio ? 'PASS' : 'WARN';
  return makeResult(rule, status, {
...
register('HSQC_SP2_PROTON_MUST_ATTACH_TO_SP2_CARBON', (rule, ctx) => {
  if (!ctx.hsqcPeaks) return skipResult(rule, 'no HSQC data');
  const violations = ctx.hsqcPeaks.filter(
    p => p.f2 >= 4.5 && p.f2 <= 8.8 && (p.f1 < 95 || p.f1 > 165)
  );
...
register('COSY_EXPECTED_VICINAL_PAIRS_MIN_RATIO', (rule, ctx) => {
  if (!ctx.cosyPeaks) return skipResult(rule, 'no COSY data');
  if (ctx.cosyPeaks.length === 0) return skipResult(rule, 'COSY data empty (0 peaks)');
  const minRatio = (rule.thresholds as Record<string, number>)?.cosy_expected_pair_min_ratio ?? 0.5;
  const expectedPairs = Math.max(1, ctx.graphFeatures.nC_protonated - 1);
  const ratio = ctx.cosyPeaks.length / expectedPairs;
  const status: RuleEvalStatus = ratio >= minRatio ? 'PASS' : ratio > 0 ? 'WARN' : 'FAIL';
  return makeResult(rule, status, {
...
register('COSY_REQUIRE_VINYLIC_CHAIN_EDGES_IF_ALKENE_CH', (rule, ctx) => {
  if (ctx.graphFeatures.nC_sp2_non_aromatic === 0) return makeResult(rule, 'PASS', { alkene_ch: 0 });
  if (!ctx.cosyPeaks) return skipResult(rule, 'no COSY data');
...
register('COSY_GRAPH_EDGE_COVERAGE_RATIO', (rule, ctx) => {
  ...
  const minRatio = ctx.snrClass === 'lowSNR'
    ? ((rule.thresholds as Record<string, number>)?.min_ratio_lowSNR ?? 0.15)
    : ((rule.thresholds as Record<string, number>)?.min_ratio_default ?? 0.3);
...
register('COSY_AROMATIC_ORTHO_EDGE_EXPECTATION', (rule, ctx) => {
  if (ctx.graphFeatures.nC_aromatic === 0) return makeResult(rule, 'PASS', { aromatic: false });
  if (!ctx.cosyPeaks) return skipResult(rule, 'no COSY data');
...
register('HMBC_CARBONYL_REQUIRED_IF_PATH_EXISTS', (rule, ctx) => {
  if (!ctx.graphFeatures.has_carbonyl) return makeResult(rule, 'PASS', { carbonyl_in_structure: false });
  if (!ctx.hmbcPeaks) return skipResult(rule, 'no HMBC data');
...
register('HMBC_AROMATIC_SUBSTITUTION_PATTERN_CHECK', (rule, ctx) => {
  if (ctx.graphFeatures.nC_aromatic === 0) return makeResult(rule, 'PASS', { aromatic_ring_count: 0 });
  if (!ctx.hmbcPeaks) return skipResult(rule, 'no HMBC data');
...
register('HMBC_CARBONYL_ALPHA_H_GATING_REQUIRED', (rule, ctx) => {
  if (!ctx.graphFeatures.has_carbonyl) return makeResult(rule, 'PASS', { carbonyl: false });
  if (!ctx.hmbcPeaks) return skipResult(rule, 'no HMBC data');
...
register('HMBC_PATH_COVERAGE_MIN_RATIO', (rule, ctx) => {
  if (!ctx.hmbcPeaks) return skipResult(rule, 'no HMBC data');
  if (ctx.hmbcPeaks.length === 0) return skipResult(rule, 'HMBC data empty (0 peaks)');
  const minRatio = ctx.snrClass === 'lowSNR'
    ? ((rule.thresholds as Record<string, number>)?.min_ratio_lowSNR ?? 0.15)
    : ((rule.thresholds as Record<string, number>)?.min_ratio_default ?? 0.3);
...
register('NOESY_EMPTY_IS_NA_IF_NOT_PROVIDED', (rule, ctx) => {
  if (!ctx.noesyPeaks) return makeResult(rule, 'SKIP', { reason: 'NOESY not provided — N/A' });
  if (ctx.noesyPeaks.length === 0) return makeResult(rule, 'WARN', { note: 'NOESY supplied but 0 crosspeaks' });
  return makeResult(rule, 'PASS', { noesy_crosspeaks: ctx.noesyPeaks.length });
});

register('NOESY_DISTANCE_PRIOR_CHECK', (rule, ctx) => {
  if (!ctx.noesyPeaks || ctx.noesyPeaks.length === 0) return skipResult(rule, 'no NOESY data');
  return makeResult(rule, 'INFO', {
...
register('IR_FUNCTIONAL_GROUP_BAND_REQUIRED', (rule, ctx) => {
  if (!ctx.ftirPeaks) return skipResult(rule, 'no IR data');
  const checks: Array<{ group: string; expected: boolean; found: boolean; region: string }> = [];
  ...
  const status: RuleEvalStatus = missingHard.length === 0 ? 'PASS' : 'WARN';
...
register('IR_CARBONYL_TYPE_DISCRIMINATION', (rule, ctx) => {
  if (!ctx.graphFeatures.has_carbonyl) return makeResult(rule, 'PASS', { has_carbonyl: false });
  if (!ctx.ftirPeaks) return skipResult(rule, 'no IR data');
...
register('IR_OH_BROAD_REQUIRED_IF_OH_PRESENT', (rule, ctx) => {
  if (ctx.graphFeatures.nH_exchangeable === 0) return makeResult(rule, 'PASS', { exchangeable_OH: 0 });
  if (!ctx.ftirPeaks) return skipResult(rule, 'no IR data');
...
register('MS_ISOTOPE_HALOGEN_SIGNATURE_REQUIRED', (rule, ctx) => {
  if (!ctx.graphFeatures.has_halogen) return makeResult(rule, 'PASS', { has_halogen: false });
  if (!ctx.msPeaks || ctx.msPeaks.length < 2) return skipResult(rule, 'insufficient MS data for isotope analysis');
  ...
  if (nCl === 1) expectedM2Ratio = 0.33;
  else if (nCl === 2) expectedM2Ratio = 0.65;
  else if (nBr === 1) expectedM2Ratio = 0.97;
  else if (nBr === 2) expectedM2Ratio = 1.0;
  else if (nCl === 1 && nBr === 1) expectedM2Ratio = 1.3;

  const ratioFit = expectedM2Ratio > 0
    ? Math.abs(m2Intensity - expectedM2Ratio) / expectedM2Ratio
    : 1.0;

  const status: RuleEvalStatus = ratioFit < 0.5 ? 'PASS' : ratioFit < 1.0 ? 'WARN' : 'FAIL';
  return makeResult(rule, status, {
    has_halogen: true,
    nCl, nBr,
...
register('MS_ISOTOPE_CLUSTER_MATCH_REQUIRED', (rule, ctx) => {
  if (!ctx.msPeaks || ctx.msPeaks.length < 2) return skipResult(rule, 'insufficient MS data');
  const sorted = [...ctx.msPeaks].sort((a, b) => b.intensity - a.intensity);
  const basePeak = sorted[0];
  ...
  const status: RuleEvalStatus = m1Error < 0.5 ? 'PASS' : m1Error < 1.5 ? 'WARN' : 'FAIL';
  return makeResult(rule, status, {
...
register('MS_ISOTOPE_ENVELOPE_M_PLUS1_M_PLUS2_FIT', (rule, ctx) => {
  ...
  const status: RuleEvalStatus = avgErr < 0.5 ? 'PASS' : avgErr < 1.5 ? 'WARN' : 'FAIL';
  return makeResult(rule, status, {
    base_peak_mz: basePeak.mz,
...
register('MS_NEUTRAL_LOSS_H2O_REQUIRES_OH_OR_COOH', (rule, ctx) => {
  ...
  if (!h2oLoss) return makeResult(rule, 'PASS', { h2o_loss_detected: false });
...
register('MS_NEUTRAL_LOSS_MEOH_REQUIRES_METHOXY', (rule, ctx) => {
  ...
  if (!meohLoss) return makeResult(rule, 'PASS', { meoh_loss_detected: false });
  return makeResult(rule, 'INFO', {
...
register('MS_ADDUCT_SET_COVERAGE', (rule, ctx) => {
  if (!ctx.msPeaks) return skipResult(rule, 'no MS data');
  return makeResult(rule, 'INFO', {
...
register('OBS_FID_COMPLEX_DATA_PRESENT', (rule, ctx) => {
  const supplied = pickMeta<boolean>(ctx, 'raw_complex_fid_present');
  if (supplied === undefined) return skipResult(rule, 'observed raw FID metadata missing');
...
register('OBS_ZERO_FILL_FACTOR_MIN', (rule, ctx) => {
  const zf = pickMeta<number>(ctx, 'zero_fill_factor');
  if (zf === undefined) return skipResult(rule, 'zero_fill_factor missing');
  return makeResult(rule, zf >= 1 ? 'PASS' : 'WARN', { zero_fill_factor: zf, min_expected: 1 });
});

register('OBS_APODIZATION_DECLARED', (rule, ctx) => {
  const apo = pickMeta<string>(ctx, 'apodization_type');
  return makeResult(rule, apo ? 'PASS' : 'INCONCLUSIVE', { apodization_type: apo ?? null });
});

register('OBS_FOURIER_TRANSFORM_APPLIED', (rule, ctx) => {
  const fft = (ctx.meta?.fft_applied as boolean | undefined);
  if (fft === undefined) return skipResult(rule, 'fft_applied missing');
  return makeResult(rule, fft ? 'PASS' : 'FAIL', { fft_applied: fft });
});

register('OBS_PHASE_CORRECTION_RESIDUAL_MIN', (rule, ctx) => {
  const residual = ctx.meta?.residual_phase_error_metric as number | undefined;
  if (residual === undefined) return skipResult(rule, 'residual phase metric missing');
...
register('OBS_BASELINE_FLATNESS_MAX', (rule, ctx) => {
  const rms = pickMeta<number>(ctx, 'baseline_rms');
  if (rms === undefined) return skipResult(rule, 'baseline_rms missing');
...
register('OBS_REFERENCE_ALIGNMENT_WITH_STANDARD', (rule, ctx) => {
  const target = pickMeta<number>(ctx, 'reference_target_ppm');
  const observed = pickMeta<number>(ctx, 'observed_reference_ppm');
  if (target === undefined || observed === undefined) return skipResult(rule, 'reference ppm metadata missing');
...
register('OBS_LINEWIDTH_QC_WITHIN_EXPECTED', (rule, ctx) => {
  const lw = ctx.meta?.linewidth_hz as number | undefined;
  if (lw === undefined) return skipResult(rule, 'linewidth_hz missing');
  return makeResult(rule, lw <= 3.5 ? 'PASS' : 'WARN', { linewidth_hz: lw, max_expected_hz: 3.5 });
});

register('OBS_SNR_MIN_FOR_ASSIGNMENT', (rule, ctx) => {
  const cls = pickMeta<string>(ctx, 'snr_class', ctx.snrClass);
  if (!cls) return skipResult(rule, 'snr_class missing');
  return makeResult(rule, cls === 'lowSNR' ? 'WARN' : 'PASS', { snr_class: cls });
});

register('OBS_DYNAMIC_RANGE_NOT_CLIPPED', (rule, ctx) => {
  const clipped = ctx.meta?.clipping_flag as boolean | undefined;
  if (clipped === undefined) return skipResult(rule, 'clipping_flag missing');
  return makeResult(rule, clipped ? 'FAIL' : 'PASS', { clipping_flag: clipped });
});

register('OBS_SOLVENT_SUPPRESSION_ARTIFACT_FLAG', (rule, ctx) => {
  const flag = ctx.meta?.solvent_suppression_flag as boolean | undefined;
  if (flag === undefined) return skipResult(rule, 'solvent_suppression_flag missing');
  return makeResult(rule, flag ? 'WARN' : 'PASS', { solvent_suppression_flag: flag });
});

register('OBS_PEAK_PICKING_REPRODUCIBILITY', (rule, ctx) => {
  const reproducible = ctx.meta?.peak_picking_reproducible as boolean | undefined;
  if (reproducible === undefined) return skipResult(rule, 'peak picking reproducibility missing');
...
register('OBS_PROCESSED_FROM_RAW_OR_DECLARED_IMPORT', (rule, ctx) => {
  const fromRaw = (ctx.meta?.processed_from_raw as boolean | undefined);
  const importDeclared = (ctx.meta?.declared_import_pipeline as boolean | undefined);
  if (fromRaw === undefined && importDeclared === undefined) return skipResult(rule, 'provenance process metadata missing');
...
function registerMetadataPresenceRule(ruleId: string, key: keyof EvalMetadataContract, failStatus: RuleEvalStatus = 'INCONCLUSIVE') {
  register(ruleId, (rule, ctx) => {
    const value = pickMeta<unknown>(ctx, key);
...
register('GLOBAL_OBSERVED_SIMULATED_PROVENANCE_SEPARATION', (rule, ctx) => {
  const prov = pickMeta<string>(ctx, 'provenance_type');
  if (!prov) return makeResult(rule, 'INCONCLUSIVE', { provenance_type: null });
...
function registerInfoRule(ruleId: string, extractor?: (ctx: EvalContext) => Record<string, unknown>) {
  register(ruleId, (rule, ctx) => makeResult(rule, 'INFO', extractor ? extractor(ctx) : { note: 'Scenario-aware rule logged for traceability' }));
}

function registerWarnRule(ruleId: string, extractor?: (ctx: EvalContext) => Record<string, unknown>) {
...
for (const id of formulaExtendedRules) registerInfoRule(id, (ctx) => ({ formula_ref: ctx.formulaRef ?? null, ion_mode: ctx.ionMode ?? null }));

register('FORMULA_MOLECULAR_WEIGHT_PLAUSIBILITY', (rule, ctx) => {
  const mw = (ctx.meta?.mw_calculated as number | undefined) ?? (ctx.meta?.exact_mass_neutral as number | undefined);
  if (mw === undefined) return skipResult(rule, 'mw metadata missing');
  if (mw < 10 || mw > 5000) return makeResult(rule, 'ERROR', { mw_calculated: mw, range: '10-5000' });
...
register('FORMULA_ATOM_COUNT_UPPER_LIMIT', (rule, ctx) => {
  ...
  if (heavy >= 100) return makeResult(rule, 'ERROR', { heavy_atom_count: heavy, max: 100 });
...
register('FORMULA_HETEROATOM_RATIO_CHECK', (rule, ctx) => {
  ...
  if (c <= 0) return makeResult(rule, 'INCONCLUSIVE', { C_count: c, heteroatom_count: hetero });
...
register('FORMULA_NITROGEN_PARITY_RULE', (rule, ctx) => {
  ...
  const nominal = ctx.meta?.mw_nominal as number | undefined;
  if (nominal === undefined) return skipResult(rule, 'mw_nominal missing');
  const oddN = n % 2 === 1;
...
register('FORMULA_ISOTOPE_LABEL_FLAG', (rule, ctx) => {
  ...
  if (isoAtoms.length && !label) return makeResult(rule, 'WARN', { isotope_atoms: isoAtoms, isotope_label: label });
...
for (const id of oneHRules) registerInfoRule(id, (ctx) => ({ h1_peak_count: ctx.h1?.peaks?.length ?? 0, solvent: ctx.solvent ?? null }));
for (const id of [
...
for (const id of c13Rules) registerInfoRule(id, (ctx) => ({ c13_peak_count: ctx.c13Peaks?.length ?? ctx.c13Signals ?? 0 }));
for (const id of [
...
for (const id of hsqcRules) registerInfoRule(id, (ctx) => ({ hsqc_peak_count: ctx.hsqcPeaks?.length ?? 0, edited_hsqc_flag: pickMeta<boolean>(ctx, 'edited_hsqc_flag') ?? null }));
for (const id of [
...
register('HSQC_TOLERANCE_PPM_WINDOW_CONSISTENCY', (rule, ctx) => {
  const field = pickMeta<number>(ctx, 'field_mhz', ctx.fieldMHz);
  if (!field) return makeResult(rule, 'ERROR', { field_mhz: null, note: 'field metadata required' });
...
for (const id of cosyRules) registerInfoRule(id, (ctx) => ({ cosy_peak_count: ctx.cosyPeaks?.length ?? 0, snr_class: ctx.snrClass ?? null }));
for (const id of [
...
for (const id of hmbcRules) registerInfoRule(id, (ctx) => ({ hmbc_peak_count: ctx.hmbcPeaks?.length ?? 0 }));
for (const id of [
...
for (const id of noesyRules) registerInfoRule(id, (ctx) => ({ noesy_peak_count: ctx.noesyPeaks?.length ?? 0, mixing_time_ms: pickMeta<number>(ctx, 'mixing_time_ms') ?? null }));
for (const id of [
...
]) registerInfoRule(id, (ctx) => ({ noesy_peak_count: ctx.noesyPeaks?.length ?? 0, mixing_time_ms: pickMeta<number>(ctx, 'mixing_time_ms') ?? null }));
...
for (const id of irRules) registerInfoRule(id, (ctx) => ({ ir_peak_count: ctx.ftirPeaks?.length ?? 0, sampling_mode: pickMeta<string>(ctx, 'sampling_mode') ?? null }));
for (const id of [
...
for (const id of msRules) registerInfoRule(id, (ctx) => ({ ms_peak_count: ctx.msPeaks?.length ?? 0, ion_mode: ctx.ionMode ?? pickMeta<string>(ctx, 'ion_mode') ?? null }));
for (const id of [
...
register('MS_HRMS_EXACT_MASS_PPM_ERROR', (rule, ctx) => {
  const observed = ctx.meta?.exact_mass_observed as number | undefined;
  const theo = ctx.meta?.exact_mass_theoretical as number | undefined;
  const resolution = ctx.meta?.ms_resolution as number | undefined;
  if (observed === undefined || theo === undefined) return skipResult(rule, 'exact mass pair missing');
  const ppm = Math.abs((observed - theo) / theo) * 1e6;
  if (resolution && resolution < 10000) return makeResult(rule, 'INFO', { ppm_error: ppm, ms_resolution: resolution, note: 'LRMS mode' });
...
registerWarnRule('GLOBAL_MODALITY_COVERAGE_MINIMUM', (ctx) => {
  const modalities = ctx.meta?.supplied_modalities as string[] | undefined;
...
register('GLOBAL_SOLVENT_FIELD_METADATA_REQUIRED', (rule, ctx) => {
  const solvent = pickMeta<string>(ctx, 'solvent_name', ctx.solvent);
  const field = pickMeta<number>(ctx, 'field_mhz', ctx.fieldMHz);
  if (!solvent) return makeResult(rule, 'ERROR', { solvent_name: null, field_mhz: field ?? null });
  if (!field) return makeResult(rule, 'WARN', { solvent_name: solvent, field_mhz: null });
  return makeResult(rule, 'PASS', { solvent_name: solvent, field_mhz: field });
});
register('GLOBAL_CONFIDENCE_AGGREGATION_PROTOCOL', (rule, ctx) =>
  makeResult(rule, 'PASS', {
...
register('GLOBAL_FILE_PROTOCOL_GRAPH_FALLBACK_REQUIRED', (rule, ctx) => {
  ...
  if (!fileProtocol) return makeResult(rule, 'PASS', { is_file_protocol: false });
...
const RULE_PREREQUISITES: Record<string, string[]> = {
...
const RULE_CONFLICTS: Array<[string, string, string]> = [
...
export function evaluateRules(
  ...
  const resultMap = new Map<string, RuleEvalResult>();
 ...
 */
export function generateCoverageReport(rules: RuleEntry[]): {
  total: number;
  implemented: number;
  missing: string[];
  coverage: number;
