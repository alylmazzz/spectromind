import type { TelemetryRuleDigest, DriftTelemetryIncident } from './telemetryTypes.js';

function severityFromStatus(status: string): DriftTelemetryIncident['severity'] {
  if (status === 'FATAL') return 'CRITICAL';
  if (status === 'ERROR') return 'ERROR';
  if (status === 'FAIL') return 'ERROR';
  if (status === 'WARN' || status === 'INCONCLUSIVE') return 'WARN';
  return 'INFO';
}

function domainFromRule(ruleId: string): DriftTelemetryIncident['domain'] {
  if (ruleId.startsWith('OBS_')) return 'PARITY';
  if (ruleId.startsWith('GLOBAL_')) return 'AUTHORITY';
  if (ruleId.startsWith('HSQC_') || ruleId.startsWith('COSY_') || ruleId.startsWith('HMBC_') || ruleId.startsWith('NOESY_')) return 'MODALITY';
  return 'VERDICT';
}

function incidentCodeFromRule(ruleId: string, status: string): string {
...
export function mapRuleDigestToIncident(digest: TelemetryRuleDigest): DriftTelemetryIncident | null {
