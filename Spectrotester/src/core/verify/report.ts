...
export function buildReport(inp: ReportInput): Record<string, unknown> {
  const report: Record<string, unknown> = {
    ...
    title_source: (inp.analysis_sot?.title_source as string | undefined) ?? 'verification_report',
    body_source: (inp.analysis_sot?.body_source as string | undefined) ?? 'verification_report',
    summary_source: (inp.analysis_sot?.summary_source as string | undefined) ?? 'verification_report',
    ...
    root_cause_analysis: inp.ruleResults
      .filter((r) => (r.status as string) === 'FAIL' || (r.status as string) === 'FATAL' || (r.status as string) === 'ERROR')
      .map((r) => ({
    ...
    skipped_rules_summary: inp.ruleResults
      .filter((r) => (r.status as string) === 'SKIP' || (r.status as string) === 'NOT_APPLICABLE')
      .map((r) => ({
        rule_id: r.rule_id,
        status: r.status,
        reason: r.evidence?.reason ?? null,
      })),
    modality_confidence_breakdown: Object.entries(
      inp.ruleResults.reduce((acc, r) => {
        const modality =
          (r.evidence?.modality as string | undefined) ??
          r.rule_id.split('_')[0];
        if (!acc[modality]) acc[modality] = { total: 0, pass: 0, warnOrWorse: 0 };
        acc[modality].total += 1;
        if (r.status === 'PASS' || r.status === 'INFO') acc[modality].pass += 1;
        else if (r.status !== 'SKIP' && r.status !== 'NOT_APPLICABLE') acc[modality].warnOrWorse += 1;
        return acc;
      }, {} as Record<string, { total: number; pass: number; warnOrWorse: number }>)
    ).map(([modality, v]) => ({
      modality,
      total_rules: v.total,
      pass_like: v.pass,
    ...
    provenance_summary: {
      ...
      authority_tier: (inp.analysis_sot?.authority_source as string | undefined) ?? null,
      engine_used: inp.engine_used ?? 'spectrotester',
  ...
  if (!valid.ok) (report as Record<string, unknown>)._validationError = valid.error;
  return report;
}

export function featureSnapshotFromGraph(f: GraphFeatures): Record<string, unknown> {
  return {
