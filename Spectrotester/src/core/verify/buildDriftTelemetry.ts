...
import type { DriftTelemetryV1, TelemetryRuleDigest } from './telemetryTypes.js';
...
export function buildDriftTelemetry(inp: BuildDriftTelemetryInput): DriftTelemetryV1 {
  const authorityTier = String(inp.context?.authority_tier ?? inp.analysisSot?.authority_source ?? 'UNKNOWN');
  const authorityGateMode = String(inp.trace?.authority_gate_mode ?? 'ALLOW') as 'ALLOW' | 'CLASS_LEVEL_ONLY' | 'HARD_BLOCK';
  ...
  const blockedModalities = Array.isArray(inp.context?.helper_contamination_blocked_modalities)
    ? (inp.context?.helper_contamination_blocked_modalities as unknown[])
    : [];
  ...
  const parityState: DriftTelemetryV1['parity_metrics']['parity_state'] =
  ...
  const digests: TelemetryRuleDigest[] = inp.ruleResults.map((r) => ({
    rule_id: r.rule_id,
    status: r.status,
    why: r.why,
  ...
  return {
    ...
    authority_metrics: {
      ...
      authority_integrity_score: Math.max(
        0,
        100 -
