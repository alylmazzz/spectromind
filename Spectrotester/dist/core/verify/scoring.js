...
export function aggregateScoring(results, policy) {
    ...
    for (const r of results) {
        const st = r.status;
        const metadataOnly = r.evidence?.metadata_only === true ||
        ...
        if (st === 'FATAL' || st === 'ERROR') {
            vetoCount++;
            if (!metadataOnly)
                penalty += penalties[st] ?? penalties['ERROR'];
            if (isObservedQc)
                observedQcCapApplied = true;
        }
        else if (st === 'FAIL') {
            failCount++;
            if (!metadataOnly)
                penalty += penalties['FAIL'];
            if (isObservedQc)
                observedQcCapApplied = true;
        }
        else if (st === 'WARN') {
            ...
            notEvaluatedCount++;
            if (!metadataOnly)
                penalty += penalties['INCONCLUSIVE'];
            if (isObservedQc)
                observedQcCapApplied = true;
        }
        ...
        if ((r.rule_id === 'GLOBAL_FALLBACK_NOT_AUTHORITATIVE' ||
    ...
    if (observedQcCapApplied)
        confidenceScore = Math.min(confidenceScore, 65);
    if (fallbackGuardFailed)
    ...
    if (vetoCount > 0)
        ...
        overall = 'FAIL';
    else if (fallbackGuardFailed)
        overall = 'INCONCLUSIVE';
    else if (notEvaluatedCount > evaluatedRules * 0.5)
        overall = 'INCONCLUSIVE';
    else if (warnCount > 0)
        overall = 'WARN';
    return {
