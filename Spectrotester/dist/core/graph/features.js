 ...
 */
export function computeGraphFeatures(graph) {
    ...
    for (const b of bonds) {
        ...
        if (order >= 2 && isC(b.beginAtomIdx) && isO(b.endAtomIdx))
            nC_carbonyl++;
        if (order >= 2 && isC(b.endAtomIdx) && isO(b.beginAtomIdx))
            nC_carbonyl++;
    }
    for (let i = 0; i < atoms.length; i++) {
        const a = atoms[i];
    ...
    const carbonSignatures = new Set();
    for (let i = 0; i < atoms.length; i++) {
        const a = atoms[i];
        if (a.symbol !== 'C')
            continue;
        const neighborBonds = bonds.filter(b => b.beginAtomIdx === a.index || b.endAtomIdx === a.index);
